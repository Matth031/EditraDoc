 ...
 * - `renderer-geometry.js` : safe-zone / clamp / fit / scale (`window.__editifyGeometry`, `bind()` avant textLayout.bind).
 */
...
if (!window.__editifyGeometry) {
  throw new Error("[editify] Charger renderer-geometry.js avant renderer.js (voir index.html).");
}
...
const geometryMod = window.__editifyGeometry;
...
function getSafeZoneSize() {
  return geometryMod.getSafeZoneSize();
}
...
function getSafeZoneSizeForPage(tab, pageKey, canvases) {
  return geometryMod.getSafeZoneSizeForPage(tab, pageKey, canvases);
}
...
function readAnnotationGeometryFromDom(node, canvas) {
  return geometryMod.readAnnotationGeometryFromDom(node, canvas);
}

function clamp(value, min, max) {
  return geometryMod.clamp(value, min, max);
}

function fitAnnotationToSafeZone(item, zone) {
  geometryMod.fitAnnotationToSafeZone(item, zone);
}

function scaleAnnotationsForZoneChange(tab, zone) {
  return geometryMod.scaleAnnotationsForZoneChange(tab, zone);
}

function scaleAnnotationsForPage(tab, zone, pageKey) {
  return geometryMod.scaleAnnotationsForPage(tab, zone, pageKey);
}

function enforceSafeZoneForActiveTab() {
  geometryMod.enforceSafeZoneForActiveTab();
}
...
geometryMod.bind({
  ...
  syncPropertyInputs: () => syncPropertyInputs(),
